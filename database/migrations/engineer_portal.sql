-- Migration: engineer portal account, assignments, and field monitoring tables.
...
INSERT IGNORE INTO engineer_project_assignments (engineer_id, project_id, assignment_notes, status)
SELECT u.id, p.id, 'Initial field inspection assignment', 'active'
FROM users u
JOIN projects p ON p.status IN ('delayed', 'active', 'assigned', 'awarded', 'approved', 'planning')
WHERE u.username = 'engineer'
  AND NOT EXISTS (
      SELECT 1
      FROM engineer_project_assignments a
      WHERE a.engineer_id = u.id
  )
ORDER BY FIELD(p.status, 'delayed', 'active', 'assigned', 'awarded', 'approved', 'planning'), p.id
LIMIT 5;

INSERT INTO engineer_issue_reports (project_id, engineer_id, issue_type, priority, description, recommended_action, status)
SELECT p.id,
       u.id,
       'Quality Concern',
       'high',
       'Sample inspection note for engineer issue reporting.',
       'Review work area and require contractor response.',
       'open'
FROM users u
JOIN projects p ON p.project_code = 'PRJ-001'
WHERE u.username = 'engineer'
  AND NOT EXISTS (
      SELECT 1
      FROM engineer_issue_reports i
      WHERE i.project_id = p.id
        AND i.engineer_id = u.id
        AND i.description = 'Sample inspection note for engineer issue reporting.'
  );
